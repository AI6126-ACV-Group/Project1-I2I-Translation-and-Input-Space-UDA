...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a source-only ResNet-18 baseline for digit UDA."
    )
    ...
    parser.add_argument("--output-dir", type=str, default="./outputs/source_only/mnist_to_usps")
...
def main() -> None:
    ...
    src_test_loss, src_test_acc = evaluate_classifier(
        model=model,
        loader=loaders["src_test"],
        criterion=criterion,
        device=device,
    )
    ...
    metrics = {
        ...
        "pretrained": args.pretrained,
        "best_val_acc": best_val_acc,
        "source_test_loss": src_test_loss,
    ...
    print(f"Source test accuracy ({args.source}): {src_test_acc * 100:.2f}%")
    print(f"Target test accuracy ({args.target}): {tgt_test_acc * 100:.2f}%")
